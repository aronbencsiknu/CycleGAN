resize = [128,128];
%load domain A
datasetFolder = fullfile("affectNET","neutral");
...
% load domain B
datasetFolder = fullfile("affectNET","surprise");
imds = imageDatastore(datasetFolder,IncludeSubfolders=true);
augmenter = imageDataAugmenter(RandXReflection=true);
...
for epoch = 1:numEpochs
    ...
    while hasdata(mbqA)
        ...
        if mod(iteration,validationFrequency) == 0 || iteration == 1
            ...
            imwrite(I, fullfile("output", "emotion", append(int2str(iteration), "_original.png")));
            xticklabels([]);
            yticklabels([]);
            title("Original");
            ...
            subplot(2,2,2);
            image(imageAxes1,I)
            imwrite(I, fullfile("output", "emotion", append(int2str(iteration), "_modified.png")));
            xticklabels([]);
            yticklabels([]);
            title("Manipulated");
        end
        if epoch > 240
            netG_BA_emotion = netG_BA;
            netG_AB_emotion = netG_AB;
            netD_A_emotion = netD_A;
            netD_B_emotion = netD_B;

            % save trained networks
            save("netG_BA_emotion.mat", "netG_BA_emotion");
            save("netG_AB_emotion.mat", "netG_AB_emotion");
            save("netD_A_emotion.mat", "netD_A_emotion");
            save("netD_B_emotion.mat", "netD_B_emotion");
        end
